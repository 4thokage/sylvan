...
export const GET: RequestHandler = async ({ locals, url }) => {
  ...
  if (!userId) {
    return json({ success: false, error: { message: 'userId parameter required' } }, { status: 400 });
  }
  ...
  const avgResult = await supabase
    .from('trade_ratings')
    .select('rating')
    .eq('rated_id', userId);
  ...
  return json({ success: true, data: { ratings: ratings || [], averageRating, totalRatings: allRatings.length } });
};
