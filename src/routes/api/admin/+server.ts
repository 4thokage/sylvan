...
export const GET: RequestHandler = async ({ locals }) => {
  ...
  const [usersResult, tradesResult, cardsResult] = await Promise.all([
    supabase.from('users').select('id, display_name, username, is_public, is_admin, created_at').order('created_at', { ascending: false }).limit(100),
    supabase.from('trades').select('id, status, created_at').order('created_at', { ascending: false }).limit(100),
